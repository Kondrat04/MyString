...
const char * MyString::SubStr(t_dword index, t_dword count)
{
    if (index >= a_length)
    {
        return nullptr;
...
t_dword MyString::Find(const char *substring, t_dword index) const
{
...
t_dword MyString::Find(const char *substring) const
{
    return Find(substring, 0);
}

t_dword MyString::Find(std::string substring, t_dword index) const
{
    return Find(substring.c_str(), index);
}

t_dword MyString::Find(std::string substring) const
{
